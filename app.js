...
game.party.push(pokemon[129], pokemon[92], pokemon[149]);
...
*/
game.party.splice(0, 2, pokemon[4], pokemon[93]);
// console.log(game.party);
...
//   }
// })
const starterPokemonPrinter = () => {
  for(z = 0; z < pokemon.length; z++){
    if(pokemon[z].starter === true){
      console.log(pokemon[z].name);
    }
  }
}
starterPokemonPrinter();
console.log();
...
game.catchPokemon = (pokemonObj) => {
  game.party.push(pokemonObj);
  for(z = 0; z < game.items.length; z++){
    if(game.items[z].name === 'pokeball')
    game.items[z].quantity -= 1;
  }
  }
